...
EXPERIMENT_NAME = "parameter_robustness"
...
SWEEPS = {
    "coarse_voxel_size": ("voxel_size", [0.6, 0.8, 1.0]),
    "ransac_distance_factor": ("distance_threshold_factor", [1.5, 2.0, 2.5]),
    "ransac_trials": ("ransac_trials", [1, 3, 5]),
    "gicp_voxel_size": ("gicp_downsampling_resolution", [0.6, 0.8, 1.0]),
}
...
def run_experiment(config: Mapping[str, Any], task: Mapping[str, Any]) -> tuple[Path, list[dict[str, Any]]]:
    ...
    for sweep_name, (param_name, values) in SWEEPS.items():
        for value in values:
            params = dict(BASE_PARAMS)
            params[param_name] = value
            variant_name = f"{sweep_name}_{value}"
            records.append(_run_variant(config, task, run_dir, variant_name, params, sweep_name, param_name, value))
...
def _run_variant(
    ...
    record = run_pairwise_task(pair_task, _config_for_variant(config, params), output_dir=run_dir, write_report=False)
    record.update(
        {
...
def _config_for_variant(config: Mapping[str, Any], params: Mapping[str, Any]) -> dict[str, Any]:
    updated = dict(config)
    preprocessing = dict(updated.get("preprocessing", {}))
    if "voxel_size" in params:
        preprocessing["voxel_size"] = params["voxel_size"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run gicp_refine parameter robustness experiment.")
