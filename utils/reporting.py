    ...
    for row in rows:
        lines.extend(
            [
                ...
                f"- Has Valid Transform: `{row.get('has_valid_transform')}`",
                f"- Algorithm Fitness: `{row.get('algorithm_fitness')}`",
                f"- Algorithm Inlier RMSE: `{row.get('algorithm_inlier_rmse')}`",
                f"- Eval Fitness: `{row.get('eval_fitness')}`",
                f"- Eval Inlier RMSE: `{row.get('eval_inlier_rmse')}`",
                f"- Eval Median NN Distance: `{row.get('eval_median_nn_dist')}`",
                f"- Eval Trimmed Mean NN Distance: `{row.get('eval_trimmed_mean_nn_dist')}`",
                f"- Eval Overlap Ratio: `{row.get('eval_overlap_ratio')}`",
                f"- Preprocess Time: `{row.get('preprocess_time')}`",
                f"- Algorithm Time: `{row.get('algorithm_time')}`",
                f"- Evaluation Time: `{row.get('evaluation_time')}`",
                f"- Artifact Time: `{row.get('artifact_time')}`",
