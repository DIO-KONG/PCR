...
class SubmapState:
    ...
    def remember_batch(self, batch_id: str, transform: Transform) -> None:
...
class SubmapManager:
    ...
    def create_next_submap(self, batch_cloud_paths: dict[str, str]) -> SubmapState:
        ...
        overlap_batches = previous.batch_order[-int(self.submap_overlap) :]
        if not overlap_batches:
            raise RuntimeError("Cannot create next submap without overlap batches.")
